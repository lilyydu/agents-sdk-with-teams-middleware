...
public class MyAgent : AgentApplication
{
    ...
    private async Task AgentsSdkProactiveAsync(ITurnContext turnContext, ITurnState turnState, CancellationToken cancellationToken)
    {
        string conversationId = turnContext.Activity.Conversation.Id;
        var api = _teamsBot.Api.ForServiceUrl(new Uri(turnContext.Activity.ServiceUrl));
        var bot = turnContext.Activity.Recipient;
        var outgoing = new MessageActivity("[Teams SDK] Proactive message triggered from an Agents SDK handler!")
        {
            From = new TeamsConversationAccount { Id = bot.Id, Name = bot.Name }
        };

        await api.Conversations.Activities.CreateAsync(conversationId, outgoing, cancellationToken: cancellationToken);
    }
