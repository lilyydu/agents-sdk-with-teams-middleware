"""
Synthesize a teams.py ``TokenProtocol`` from an inbound Agents SDK Activity.

teams.py's ``ActivityEvent`` requires a ``TokenProtocol`` so the framework
can build its context (service_url fallback, caller classification,
app_id). The Agents SDK has already validated the inbound JWT by the time
the middleware runs, so we don't re-forward the raw bearer — we just
project the fields teams.py actually consumes off the Activity itself.

Outbound auth goes through teams.py's ``credentials`` → ``_token_manager``,
which the Teams SDK wiring sets up separately via
``credentials.make_agent_sdk_token_provider``.
"""
...
class _TeamsSDKToken(TokenProtocol):
    ...
    @classmethod
    def from_activity(cls, activity: Activity) -> "_TeamsSDKToken":
